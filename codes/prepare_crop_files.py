...
# face_cascade = cv2.CascadeClassifier('/Users/scueva/opencv/data/haarcascades/haarcascade_frontalface_default.xml')
...
# 	video_files = [f for f in os.listdir(source_path) if fnmatch.fnmatch(f, '*.mpg')]
# 	for j in range(1,len(video_files)+1):#range(1,numfiles+1):

# 		file = video_files[j-1]
...
# 		cap = cv2.VideoCapture(os.path.join(source_path, file))
...
for i in speakers:
	...
	# numfiles=len(fnmatch.filter(os.listdir(path+'/Video/s'+str(i)), '*.mpg'))
	video_files = [f for f in os.listdir(source_path) if fnmatch.fnmatch(f, '*.mpg')]
	for j in range(1,len(video_files)+1):

		file = video_files[j-1]
		format_num1="{number:06}".format(number=j)
		#os.system('ffmpeg -i '+ str(format_num1)+'.mpg' +' -q:a 0 -map a '+path+'/Audio/s'+str(i)+'/'+str(format_num1)+'.wav' )
		os.system('ffmpeg -i '+ os.path.join(source_path, file) +' -ac 1 -ar 8000 '+path+'/Audio/s'+str(i)+'/'+str(format_num1)+'.wav' )
